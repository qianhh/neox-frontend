...
import { batchData, batchDataAnytrust, batchDataCelestia } from '../../mocks/txn-batch';
...
test('base view', async({ render, mockApiResponse }) => {
  await mockApiResponse('core:arbitrum_l2_txn_batch', batchData, { pathParams: { number: batchNumber } });
  const component = await render(<ArbitrumL2TxnBatch/>, { hooksConfig });
  await expect(component).toHaveScreenshot();
});

test('with anytrust DA', async({ render, mockApiResponse }) => {
  await mockApiResponse('core:arbitrum_l2_txn_batch', batchDataAnytrust, { pathParams: { number: batchNumber } });
  const component = await render(<ArbitrumL2TxnBatch/>, { hooksConfig });
